...
from utils.get_image_list import get_image_list, get_list_from_folder
...
class SystemPredictor(object):
    ...
    def predict(self, img):
        ...
        # results = self.det_predictor.predict(img)     # 配合det模型使用
        results = []        # 仅使用 recognition模型
        ...
        for result in results:
            preds = {}
            xmin, ymin, xmax, ymax = result["bbox"].tolist()
        ...
        output[0]['rec_scores'] = float(output[0]['rec_scores'])
...
def main_(config):
    ...
    json_list = get_list_from_folder(config["Global"]["infer_imgs"])
    for json_file in json_list:
        ...
        for bbox in bboxs:
            ...
            output = system_predictor.predict(img_crop)
            # draw_bbox_results(img, output, image_file)
            print(output)
            outputs.append(output)
        tar_path = str(json_file.parent / json_file.stem) + ('new.json')
